...
export default function Cart() {
    ...
    return (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=> cartStore.toggleCart()} className="fixed w-full h-screen left-0 top-0 bg-black/25">
            <motion.div layout onClick={(e) => e.stopPropagation()} className="bg-[#F7F2EE] absolute right-0 top-0 lg:w-2/5 h-screen p-12 overflow-y-scroll text-black">
                ...
                <motion.div layout>
                    {cartStore.cart.length > 0 &&
                        <button className="bg-black text-white px-5 py-2 w-full">Checkout</button>
                    }
                </motion.div>
